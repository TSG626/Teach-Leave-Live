      ...
      <DialogTitle id="simple-dialog-title">
        <div>
          <Avatar
            round={true}
            size="3rem"
            color={Avatar.getRandomColor("sitebase", ["#2BCED6", "#00ADB5"])}
...
const GetUsers = () => {
  ...
  const makeAdmin = (user, popupState) => {
    ...
    if (user.status !== 1) {
      ...
      if (confirmed) {
        API.post("/api/admin/changeStatus", {
          email: user.email,
          status: 1,
        }).then((res) => {
  ...
  const deleteUser = (user, popupState) => {
    ...
    if (confirmed) {
      ...
      API.post("/api/admin/deleteUser", { username: user.username }).then(
        (res) => {
